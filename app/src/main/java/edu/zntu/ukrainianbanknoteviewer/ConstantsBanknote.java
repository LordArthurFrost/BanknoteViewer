package edu.zntu.ukrainianbanknoteviewer;

public class ConstantsBanknote
{
    public static final int DENOMINATION = 0;
    public static final int PRINTYEAR = 1;
    public static final int DATE = 2;
    public static final int MEMORABLE = 3;
    public static final int TURNOVER = 4;
    public static final int DESCRIPTIONFRONT = 5;
    public static final int DESCRIPTIONBACK = 6;
    public static final int PROTECTIONINFO = 7;
    public static final int EXTRAINFOINFO = 8;
    public static final int SIZEINFO = 9;
    public static final int IDINFO = 10;
    public static final int MEMORABLEPOSITION =11;
    public static final int SIZEPOSITION =12;
    public static final int TURNOVERPOSITION =13;
}
